...
public class RDFOWLReasoning {
	...
	protected static void printInferredTriples(InfModel infModel, Model originalModel) {
		...
		while (iter.hasNext()) {
			...
			if (isRelevantStatement(stmt)) { //true pour afficher toutes les inférences possibles
	...
	protected static void printTriples(Model infModel) {
	...
	private static boolean isRelevantStatement(Statement stmt) {
		...
		Set<String> Pbanned = Set.of(RDFS.getURI(), OWL2.getURI());
		
		

		Set<String> SObanned = Set.of(RDF.getURI(), RDFS.getURI(), OWL2.getURI());
